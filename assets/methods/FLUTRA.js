...
process.on('uncaughtException', (e) => { console.log('\x1b[31m' + e + '\x1b[0m') }); // Red for uncaughtException error - Copyright: @bjorqaa
process.on('unhandledRejection', (e) => { console.log('\x1b[31m' + e + '\x1b[0m') }); // Red for unhandledRejection - Copyright: @bjorqaa

// Check input parameters
if (process.argv.length < 7) {
    console.log("\x1b[35m\nHow to use: (Copyright: @bjorqaa)\x1b[0m"); // Purple for title
...
try {
    ...
    console.error(`\x1b[31m\nError: Invalid URL - ${error.message} (Copyright: @bjorqaa)\x1b[0m\n`); // Red for URL error
    process.exit(1);
}
...
if (!fs.existsSync(process.argv[6])) {
    console.error(`\x1b[31m\nError: File proxy "${process.argv[6]}" does not exist (Copyright: @bjorqaa)\x1b[0m\n`); // Red for proxy file error
    process.exit(1);
}
...
function startRequest(proxies) {
    if (!proxies || proxies.length === 0) {
        console.error("\x1b[31mNo proxy list available (Copyright: @bjorqaa)\x1b[0m"); // Red for proxy error
        return;
    }
...
if (cluster.isMaster) {
    ...
    console.log(`\x1b[95mLoaded ${proxies.length} valid proxies (Copyright: @bjorqaa)\x1b[0m`); // Pink for proxy notification


    // Check if there are no valid proxies
    if (proxies.length === 0) {
        console.error("\x1b[31mNo valid proxies found. Please check your proxy file. (Copyright: @bjorqaa)\x1b[0m"); // Red for no proxy error
        process.exit(1);
    }
    ...
    console.log(`\x1b[35mMain start :) (Copyright: @bjorqaa)\x1b[0m`); // Purple for start notification
    ...
    setInterval(() => {
        ...
        console.clear();
        console.log(`\x1b[95m${JSON.stringify(statuses)} (Copyright: @bjorqaa)\x1b[0m`); // Pink for displaying statuses
    }, 1000);
